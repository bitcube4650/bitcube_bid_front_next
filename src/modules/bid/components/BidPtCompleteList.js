...
const BidPtCompleteList = ({data}) => {
    ...
    return (
        <tr>
            <td><a href={()=>false} onClick={()=>onClickBidDetail(data.biNo)} className="textUnderline" title="입찰번호">{ data.biNo }</a></td>
            <td className="text-left"><a href={()=>false} onClick={()=>onClickBidDetail(data.biNo)} className="textUnderline" title="입찰명">{ data.biName }</a></td>
