...
const OtherCustListPop = ({setSelCust, setOtherCustModal}) => {
	...
	const onSearch = useCallback(async() => {
		...
		setOtherCustList(response.data);
	})
	...
	const selectCustCode = async(custCode) => {
		setSelCust(current => ({
			...current,
			custCode : custCode,
			url : '/api/v1/cust/otherCustManagement/'
		}))
  ...
  return (
	<div className="modal fade modalStyle" id="otherCustPop" tabIndex="-1" role="dialog" aria-hidden="true">
	{/* <div className="" id="otherCustPop" tabIndex="-1" role="dialog" aria-hidden="true"> */}
		<div className="modal-dialog" style={{width:"100%", maxWidth:"900px"}}>
			<div className="modal-content">
				<div className="modal-body">
					...
					<div className="modalFooter">
						<button className="modalBtnClose" data-dismiss="modal" title="닫기">닫기</button>
					</div>
				</div>				
			</div>
		</div>
