...
export const getServerSideProps = async (context) => {
    ...
    let userAuth = cookies.substring(cookies.indexOf("userAuth")+17);
    userAuth = userAuth.substring(0, userAuth.indexOf("%22"));

    const params1 = {
        userAuth : userAuth
    }

    let coInterList = [{} as MapType];
    try {
        axios.defaults.headers.cookie = cookies;
        const response = await axios.post("http://localhost:3000/api/v1/statistics/coInterList", params1);
        coInterList = response.data.data
    } catch (error) {
    ...
    console.log(coInterList);

    let params = {
        userAuth : userAuth					//조회조건 : 사용자권한
    ,	coInters : Array<String>()					//조회조건 : 계열사 배열
    ,	size : 10						//10개씩 보기
    ,	page : 0						//클릭한 페이지번호
    ,   startDay : Ft.strDateAddDay(Ft.getCurretDate(), -30)                  //조회조건 : 입찰완료 - 시작일
    ...
    try {
        axios.defaults.headers.cookie = cookies;
        const response = await axios.post("http://localhost:3000/api/v1/statistics/bidDetailList", params);
        return {
            props: {
