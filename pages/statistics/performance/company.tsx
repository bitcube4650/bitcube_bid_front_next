...
const company = ({ results } : {results :MapType}) => {
	...
	const [list, setList] = useState<MapType>(results.initList)
	const [listSize, setListSize] = useState<number>(results.initListCnt)
	const [listSum, setListSum] = useState<MapType>(results.initListSum)
	...
	const onMoveDetail = (interrelatedCustCode:string) =>{
		router.push({
			pathname: '/statistics/performance/detail',
			query: {
				interrelatedCustCode : interrelatedCustCode,
				srcDataStr : JSON.stringify(srcData)
			},
		},
		'/statistics/performance/detail'
		)
	}
						...
						<tbody>
							{list.map((obj:MapType,idx:number) => (
								<tr key={idx}>
									...
									<a onClick={() => { onMoveDetail(obj.interrelatedCustCode) }} className="textUnderline notiTitle">{obj.interrelatedNm}</a>
									</td>
...
export const getServerSideProps = async (context) => {
	...
	try {
		...
		const response = await axios.post("http://localhost:3000/api/v1/statistics/biInfoList", params);

		if(response.data.code === 'OK'){
			results.initList =  response.data.data.slice(0, response.data.data.length - 1)
			results.initListCnt = response.data.data.length - 1
			results.initListSum = response.data.data[response.data.data.length - 1]
		}
		return {
			props: {
				results
