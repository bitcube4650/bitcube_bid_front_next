        ...
        <div className="conRight">
            ...
            <div className="contents">
                ...
                <div className="boxSt mt20">
                    ...
                    <div className="flex align-items-center mt20">
                        ...
                        <div className="flex width100">
                            <input type="radio" name="bm2" value="ALL" id="bm2_1" className="radioStyle" checked={ dataFromList && dataFromList.bco == "ALL" } disabled={ dataFromList &&  dataFromList.bco != 'ALL'? true:false } />
                            <label htmlFor="bm2_1" className={ (dataFromList && dataFromList.bco != 'ALL')?'dimmed':'' }>공통</label>
                            <div>
                                <input type="radio" name= "bm2" value="CUST" id="bm2_2" className="radioStyle" checked={ dataFromList &&  dataFromList.bco == "CUST" } disabled={ dataFromList && dataFromList.bco != 'CUST'?true:false } />
                                <label htmlFor="bm2_2" className={ dataFromList &&  dataFromList.bco != 'CUST'?'dimmed':'' }>계열사</label>
                                { (dataFromList && dataFromList.bco == 'CUST') &&
                                    <p className="mt5 ml30">{ dataFromList && dataFromList.interrelatedNms }</p>
                                }
                            </div>
                        </div>
                    </div>
                    <div className="flex align-items-center mt20">
                        ...
                        <div className="width100">{ dataFromList ? dataFromList.buserName : ''}</div>
                    </div>
                    <div className="flex align-items-center mt20">
                        <div className="formTit flex-shrink0 width170px">공지일시</div>
                        <div className="width100">{ dataFromList ? dataFromList.bdate : '' }</div>
                    </div>
                    <div className="flex align-items-center mt20">
                        <div className="formTit flex-shrink0 width170px">조회수</div>
                        <div className="width100">{ dataFromList ? dataFromList.bcount : ''}</div>
                    </div>
                    <div className="flex align-items-center mt20">
                        <div className="formTit flex-shrink0 width170px">첨부파일</div>
                        { (dataFromList && dataFromList.bfilePath != null) && (dataFromList && dataFromList.bfilePath != '') &&
                            <div className="width100">
                                <a onClick={ onDownloadFile } id="file-download" className="textUnderline">{ dataFromList ? dataFromList.bfile : ''}</a>
                            </div>
                        }
                    </div>
                    <div className="flex mt20">
                        ...
                        <div style={{width: 'calc(100% - 170px)'}}>
                            <pre className="overflow-y-auto notiBox" style={{height:'400px', backgroundColor: 'white'}}>
                                { dataFromList ? dataFromList.bcontent : '' }
                            </pre>
                        </div>
                    </div>
                </div>
                ...
                <div className="text-center mt50">
                    <a onClick={()=>{onMoveList()}} className="btnStyle btnOutline" title="목록">목록</a>
