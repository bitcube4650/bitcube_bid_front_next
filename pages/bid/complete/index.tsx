...
const Index = ({ initList }: { initList: MapType }) => {
    ...
    useEffect(() => {
        if(!Ft.isEmpty(router.query)) {
            if(router.query.keyword == 'completed'){
    ...
    const clickBidDetail = (biNo:string) => {
        ...
        router.push('/bid/complete/detail');
    };
...
export const getServerSideProps = async (context) => {
    ...
    try {
        ...
        const response = await axios.post("http://localhost:3000/api/v1/bidComplete/list", params);
        return {
            props: {
